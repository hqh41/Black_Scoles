...
call::call(double T, double r, double sigma, double K, double L):payoff(T, r, sigma, K, L){}
matrix call::option_Crank_Nicolson(int m, int n){
    double s;
    double h  = L_/m;
    double dt = T_/n;
    double lambda, nu;
    ...
    for(k = 1; k <= n; k++){
        P = mul(C, P);
        P(0, 0) = 0;
        P(m, 0) = ((s-K_*exp(-r_*(T_-k*dt)) > 0) ? (s-K_*exp(-r_*(T_-k*dt))) : 0);
    }
    return P;
}

matrix call::option_DFI(int m, int n){
    matrix A(m + 1, m + 1);
    matrix B(m + 1, m + 1);
    matrix C(m + 1, m + 1);
    matrix P(m + 1, 1);
